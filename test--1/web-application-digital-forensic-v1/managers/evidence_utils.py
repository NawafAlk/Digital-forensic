...
# Defer heavy forensic imports so the web app can start without them installed
_FORENSICS_IMPORT_ERROR = None
try:
    from Registry import Registry
    import pyewf
    import pytsk3
except Exception as _e:
    _FORENSICS_IMPORT_ERROR = _e
...
class ImageHandler:
    def __init__(self, image_path):
        if _FORENSICS_IMPORT_ERROR is not None:
            raise ImportError(
                "Forensic backends missing: please install 'python-registry', 'pytsk3', and 'libewf-python' in your venv."
            ) from _FORENSICS_IMPORT_ERROR
    ...
    def detect_file_type(self, content):
        """Detect file type based on magic numbers and content analysis."""
        if not content:
            return {'type': 'unknown', 'extension': '', 'description': 'Unknown file type'}
        
        # Magic number signatures
        signatures = {
            b'\x89PNG\r\n\x1a\n': {'type': 'image', 'extension': 'png', 'description': 'PNG Image'},
            b'\xff\xd8\xff': {'type': 'image', 'extension': 'jpg', 'description': 'JPEG Image'},
            b'GIF87a': {'type': 'image', 'extension': 'gif', 'description': 'GIF Image'},
            b'GIF89a': {'type': 'image', 'extension': 'gif', 'description': 'GIF Image'},
            b'BM': {'type': 'image', 'extension': 'bmp', 'description': 'Bitmap Image'},
            b'%PDF': {'type': 'document', 'extension': 'pdf', 'description': 'PDF Document'},
            b'PK\x03\x04': {'type': 'archive', 'extension': 'zip', 'description': 'ZIP Archive'},
            b'PK\x05\x06': {'type': 'archive', 'extension': 'zip', 'description': 'ZIP Archive'},
            b'PK\x07\x08': {'type': 'archive', 'extension': 'zip', 'description': 'ZIP Archive'},
            b'\x1f\x8b': {'type': 'archive', 'extension': 'gz', 'description': 'GZIP Archive'},
            b'Rar!\x1a\x07\x00': {'type': 'archive', 'extension': 'rar', 'description': 'RAR Archive'},
            b'7z\xbc\xaf\x27\x1c': {'type': 'archive', 'extension': '7z', 'description': '7-Zip Archive'},
            b'MZ': {'type': 'executable', 'extension': 'exe', 'description': 'Windows Executable'},
            b'\x7fELF': {'type': 'executable', 'extension': 'elf', 'description': 'Linux Executable'},
            b'\xfe\xed\xfa': {'type': 'executable', 'extension': 'mach', 'description': 'macOS Executable'},
            b'<!DOCTYPE html': {'type': 'web', 'extension': 'html', 'description': 'HTML Document'},
            b'<html': {'type': 'web', 'extension': 'html', 'description': 'HTML Document'},
            b'<?xml': {'type': 'document', 'extension': 'xml', 'description': 'XML Document'},
            b'{\n': {'type': 'data', 'extension': 'json', 'description': 'JSON Data'},
            b'[': {'type': 'data', 'extension': 'json', 'description': 'JSON Array'},
        }
        ...
        return {'type': 'unknown', 'extension': 'bin', 'description': 'Binary File'}

    def carve_files_by_type(self, file_type='all'):
        """Carve files of specific type from the disk image."""
        carved_files = []
        ...
        type_mappings = {
            ...
            'web': ['html', 'htm', 'css', 'js', 'php', 'asp'],
            'data': ['json', 'xml', 'csv', 'sql', 'db'],
            'all': []
        }
        
        extensions = type_mappings.get(file_type, [])
        ...
        try:
            ...
            for partition in volume_info:
                if partition.flags == pytsk3.TSK_VS_PART_FLAG_ALLOC:
                    self._carve_from_partition(img_info, partition.start * SECTOR_SIZE, 
                                             carved_files, extensions, file_type)
        except Exception as e:
            print(f"Error during carving: {e}")
        
        return carved_files

    def _carve_from_partition(self, img_info, offset, carved_files, extensions, file_type):
        """Carve files from a specific partition."""
        try:
            fs_info = pytsk3.FS_Info(img_info, offset=offset)
            self._recursive_carve(fs_info, fs_info.open_dir(path="/"), "/", 
                                carved_files, extensions, file_type)
        except Exception as e:
            print(f"Unable to carve from partition at offset {offset}: {e}")

    def _recursive_carve(self, fs_info, directory, parent_path, carved_files, extensions, file_type):
        """Recursively carve files from directory structure."""
        for entry in directory:
            if entry.info.name.name in [b".", b".."]:
                continue
            ...
            if entry.info.meta and entry.info.meta.type == pytsk3.TSK_FS_META_TYPE_DIR:
                try:
                    sub_directory = fs_info.open_dir(inode=entry.info.meta.addr)
                    self._recursive_carve(fs_info, sub_directory, 
                ...
                # Check if file matches carving criteria
                if self._should_carve_file(file_extension, extensions, file_type):
                    file_info = self._get_carved_file_info(entry, parent_path, fs_info)
                    if file_info:
                        carved_files.append(file_info)

    def _should_carve_file(self, file_extension, extensions, file_type):
        """Determine if file should be carved based on type criteria."""
        if file_type == 'all':
            return True
    ...
    def _get_carved_file_info(self, entry, parent_path, fs_info):
        """Extract information about a carved file."""
        try:
            file_name = entry.info.name.name.decode("utf-8")
            ...
            content = file_obj.read_random(0, min(file_obj.info.meta.size, 1024))  # Read first 1KB for detection
            file_type_info = self.detect_file_type(content)
            ...
            return {
                ...
                "accessed": self._safe_datetime(entry.info.meta.atime),
                "modified": self._safe_datetime(entry.info.meta.mtime),
                "created": self._safe_datetime(entry.info.meta.crtime) if hasattr(entry.info.meta, 'crtime') else "N/A",
                "changed": self._safe_datetime(entry.info.meta.ctime),
            }
        except Exception as e:
            print(f"Error getting file info: {e}")
            return None

    def _safe_datetime(self, timestamp):
